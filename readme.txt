...
Use a voltmeter to double check, but the ground pin is on the right when looking at the Victron VE.Direct port. The +5v pin is on the far left, but it does not have enough power available to really power anything like the D1 Mini. Tx is right next to the +5V and Rx (not used here) is next to the ground.
